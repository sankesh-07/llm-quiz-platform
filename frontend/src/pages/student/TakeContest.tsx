...
export default function TakeContest() {
  ...
  useEffect(() => {
    const fetchContestAndQuiz = async () => {
      if (!id) return;
      try {
        ...
        if (contestData.status === 'LIVE') {
          try {
            const quizData = await contestsService.getContestQuiz(id);
            setQuiz(quizData);
            if (quizData.timerSeconds) {
              setTimeRemaining(quizData.timerSeconds);
            }
          } catch (err: any) {
            setError(err.response?.data?.error || 'Failed to load quiz');
          }
        } else {
          setError('Contest is not live yet');
        }
      } catch (err: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // For contest mode: do NOT redirect to per-question solutions.
      // Instead, send the student to the contest details page, where results/solutions
      // are only shown after the contest has ended.
  ...
  if (error || !quiz || !contest) {
    return (
      <div className="px-4 py-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">{error || 'Contest or quiz not found'}</p>
        </div>
      </div>
    );
  }
